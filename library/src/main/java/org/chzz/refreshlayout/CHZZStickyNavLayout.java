...
package org.chzz.refreshlayout;
...
import org.chzz.refreshlayout.util.CHZZRefreshScrollingUtil;
...
public class CHZZStickyNavLayout extends LinearLayout {
    ...
    public CHZZStickyNavLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }
    ...
    public void onFinishInflate() {
        ...
        if (getChildCount() != 3) {
            throw new IllegalStateException(CHZZStickyNavLayout.class.getSimpleName() + "必须有且只有三个子控件");
        }
    ...
    public boolean isContentViewToTop() {
        ...
        if (CHZZRefreshScrollingUtil.isScrollViewOrWebViewToTop(mDirectWebView)) {
            return true;
        }

        if (CHZZRefreshScrollingUtil.isScrollViewOrWebViewToTop(mDirectScrollView)) {
            return true;
        }

        if (CHZZRefreshScrollingUtil.isAbsListViewToTop(mDirectAbsListView)) {
            return true;
        }

        if (CHZZRefreshScrollingUtil.isRecyclerViewToTop(mDirectRecyclerView)) {
            return true;
        }
    ...
    private boolean isViewPagerContentViewToTop() {
        ...
        if (CHZZRefreshScrollingUtil.isScrollViewOrWebViewToTop(mNestedWebView)) {
            return true;
        }

        if (CHZZRefreshScrollingUtil.isScrollViewOrWebViewToTop(mNestedScrollView)) {
            return true;
        }

        if (CHZZRefreshScrollingUtil.isAbsListViewToTop(mNestedAbsListView)) {
            return true;
        }

        if (CHZZRefreshScrollingUtil.isRecyclerViewToTop(mNestedRecyclerView)) {
            return true;
        }
    ...
    private void regetNestedContentView() {
        ...
        if (adapter instanceof FragmentPagerAdapter || adapter instanceof FragmentStatePagerAdapter) {
            ...
            throw new IllegalStateException(CHZZStickyNavLayout.class.getSimpleName() + "的第三个子控件为ViewPager时，其adapter必须是FragmentPagerAdapter或者FragmentStatePagerAdapter");
        }
    }

    public void setRefreshLayout(CHZZRefreshLayout refreshLayout) {
    ...
    public boolean shouldHandleLoadingMore() {
        ...
        if (CHZZRefreshScrollingUtil.isWebViewToBottom(mDirectWebView)) {
            return true;
        }

        if (CHZZRefreshScrollingUtil.isScrollViewToBottom(mDirectScrollView)) {
            return true;
        }
        ...
        if (mDirectViewPager != null) {
            ...
            if (CHZZRefreshScrollingUtil.isWebViewToBottom(mNestedWebView)) {
                return true;
            }

            if (CHZZRefreshScrollingUtil.isScrollViewToBottom(mNestedScrollView)) {
                return true;
            }
    ...
    public void scrollToBottom() {
        CHZZRefreshScrollingUtil.scrollToBottom(mDirectScrollView);
        CHZZRefreshScrollingUtil.scrollToBottom(mDirectRecyclerView);
        CHZZRefreshScrollingUtil.scrollToBottom(mDirectAbsListView);

        if (mDirectViewPager != null) {
            if (mNestedContentView == null) {
                regetNestedContentView();
            }
            CHZZRefreshScrollingUtil.scrollToBottom(mNestedScrollView);
            CHZZRefreshScrollingUtil.scrollToBottom(mNestedRecyclerView);
            CHZZRefreshScrollingUtil.scrollToBottom(mNestedAbsListView);
        }
    }
}