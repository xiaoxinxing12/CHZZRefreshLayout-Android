 ...
 */
public class CHZZRefreshLayout extends LinearLayout {
    private static final String TAG = CHZZRefreshLayout.class.getSimpleName();

    private CHZZRefreshViewHolder mRefreshViewHolder;
    /**
    ...
    private CHZZStickyNavLayout mStickyNavLayout;
    ...
    public CHZZRefreshLayout(Context context) {
        this(context, null);
    }

    public CHZZRefreshLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public void onFinishInflate() {
        ...
        if (getChildCount() != 2) {
            throw new RuntimeException(CHZZRefreshLayout.class.getSimpleName() + "必须有且只有一个子控件");
        }
        ...
        } else if (mContentView instanceof CHZZStickyNavLayout) {
            mStickyNavLayout = (CHZZStickyNavLayout) mContentView;
    ...
    public void setRefreshViewHolder(CHZZRefreshViewHolder refreshViewHolder) {
    ...
    public boolean shouldHandleAbsListViewLoadingMore(AbsListView absListView) {
        ...
        return CHZZRefreshScrollingUtil.isAbsListViewToBottom(absListView);
    }
    ...
    public boolean shouldHandleRecyclerViewLoadingMore(RecyclerView recyclerView) {
        ...
        return CHZZRefreshScrollingUtil.isRecyclerViewToBottom(recyclerView);
    }
    ...
    private boolean shouldHandleLoadingMore() {
        ...
        if (CHZZRefreshScrollingUtil.isWebViewToBottom(mWebView)) {
            return true;
        }

        if (CHZZRefreshScrollingUtil.isScrollViewToBottom(mScrollView)) {
            return true;
        }
    ...
    private boolean isContentViewToTop() {
        ...
        if (CHZZRefreshScrollingUtil.isScrollViewOrWebViewToTop(mWebView)) {
            return true;
        }

        if (CHZZRefreshScrollingUtil.isScrollViewOrWebViewToTop(mScrollView)) {
            return true;
        }

        if (CHZZRefreshScrollingUtil.isAbsListViewToTop(mAbsListView)) {
            return true;
        }

        if (CHZZRefreshScrollingUtil.isRecyclerViewToTop(mRecyclerView)) {
            return true;
        }

        if (CHZZRefreshScrollingUtil.isStickyNavLayoutToTop(mStickyNavLayout)) {
            return true;
        }
    ...
    private void showLoadingMoreView() {
        ...
        CHZZRefreshScrollingUtil.scrollToBottom(mScrollView);
        CHZZRefreshScrollingUtil.scrollToBottom(mRecyclerView);
        CHZZRefreshScrollingUtil.scrollToBottom(mAbsListView);
        if (mStickyNavLayout != null) {
